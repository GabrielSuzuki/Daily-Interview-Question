...
# and a valid move is one placed on an empty block in the grid. A player who succeeds in placing n of
# their marks in a horizontal, diagonal, or vertical row wins the game. Once a winning condition is reached, 
...
class TicTacToe(object):
  ...
  def move(self, row, col, player):
    ...
    size = rows.length
    if (Math.abs(rows[row]) == size or
      Math.abs(cols[col]) == size or
      Math.abs(diagonal) == size  or
